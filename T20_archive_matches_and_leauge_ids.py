...
for year in years:
    ...
    for j in range(len(response_data.get('result', {}).get('competitions', []))):
        if response_data.get('result', {}).get('competitions', [])[j].get('category', "") == "T20 League":
            i = j
            ...
            # Save to CSV
            df_info.to_csv('T20_id_info.csv', index=False, mode='a', header=not pd.io.common.file_exists('T20_id_info.csv'))
